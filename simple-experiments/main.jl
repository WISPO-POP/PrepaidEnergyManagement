...
function main()
    ...
    # Type of data: 0 = perfect, 1 = 24h average, 2 = imperfect, same energy demand, 3 = imperfect, different energy demand, 3.1 = imperfect, different energy demand, averaged
    simulation_data_number = 0;
    model_data_number = 0;
    # Type of model
    model_number = 0; # 0 - detailed model, 1 - variable recharge model (assumes average data), 2 - full-fledged home energy manager (hem)
    threshold_constant_days = 1;
    #load_priority_order = [2;4;3;1]
    load_priority_order = [1;1]
    start_day = 1;
    # Add cases
    case_array = [2]
    # Add real recharge frequency over experiment duration
    real_recharge_logic_array = [1];
    # Add real recharge amount in percent
    recharge_percent_array = [75];    
...
function run_hem_model_simulation(experiment_parameters)
    ...
    metrics_results = zeros(number_of_results,9)
    metrics_results_index = 0
    model_metrics_results = zeros(number_of_results,9)
    ...
    demand_state, demand_power_W, total_demand_energy_kWh, number_of_loads, load_priority_factor, daily_energy_Wh, daily_energy_array_Wh = initialize_load_parameters(total_time_days, timestep_hours, load_priority_order, start_day, directory);             
    # Real recharge parameters
    daily_real_recharge, daily_real_cost, cost_perkWh, real_recharge_schedule, real_cost_schedule, recharge_amount = initialize_real_recharge_parameters(total_time_days, timestep_hours, total_demand_energy_kWh, recharge_percent, real_recharge_logic);
    ...
    elseif (model_data_number == 1) || (model_data_number == 2) || (model_data_number == 3) || (model_data_number == 3.1)
    ...
    model_actuation_state, model_psf, model_esf, model_load_tsf = compute_hem_schedule(total_time_days, timestep_hours, model_demand_state, model_demand_power_W, cost_perkWh, number_of_loads, load_priority_factor, daily_real_recharge, directory)
    ...
    priority_sf = 100*sum(load_priority_factor[k]*(sum(actuation_state[k,t] for t in 1:number_of_timesteps)/(sum(demand_state[k,t] for t in 1:number_of_timesteps))) for k in 1:number_of_loads)
    energy_sf = sum(sum(actuation_state[k,t]*demand_power_W[k,t] for k in 1:number_of_loads) for t in 1:number_of_timesteps)*100/sum(sum(demand_state[k,t]*demand_power_W[k,t] for k in 1:number_of_loads) for t in 1:number_of_timesteps)
    # SF per load
    time_service_factor_perload = zeros(number_of_loads,1);
    for k in 1:number_of_loads
        time_service_factor_perload[k] = (sum(actuation_state[k,t] for t in 1:number_of_timesteps))*100/(sum(demand_state[k,t] for t in 1:number_of_timesteps))     
    end    
    time_array = (0:(number_of_timesteps-1))*timestep_hours;
    # Store results
    simulation_results = Dict("actuation_state" => actuation_state, "demand_state" => demand_state, "real_wallet" => real_wallet);
    model_results = Dict("actuation_state" => model_actuation_state, "demand_state" => model_demand_state);
    parameters = Dict("number_of_loads" => number_of_loads, "number_of_timesteps" => number_of_timesteps, "demand_power_W" => demand_power_W, "load_priority_factor" => load_priority_factor, "case" => case, "recharge_percent" => recharge_percent, "real_recharge_logic" => real_recharge_logic, "timestep_hours" => timestep_hours, "model_data_number" => model_data_number, "simulation_data_number" => simulation_data_number)
    
    plot_hem_model_results(time_array, simulation_results, model_results, parameters, directory)
    ...
    metrics_results[metrics_results_index,5:6] = time_service_factor_perload
    metrics_results[metrics_results_index,7] = -1
    metrics_results[metrics_results_index,8] = energy_sf
    metrics_results[metrics_results_index,9] = disconnection_counter   
    ...
    model_metrics_results[model_metrics_results_index,5:6] = model_load_tsf
    model_metrics_results[model_metrics_results_index,7] = -1
    model_metrics_results[model_metrics_results_index,8] = model_esf
    ...
    column_names = ["case", "real_recharge_logic", "recharge_percent", "priority_sf", "load1_tsf", "load2_tsf", "time_sf", "energy_sf", "disconnections"]
    metrics_results_df = DataFrame(metrics_results, column_names)
    model_metrics_results_df = DataFrame(model_metrics_results, column_names)
...
function run_threshold_model_simulation(experiment_parameters)
    ...
    # Initialize load parameters
    demand_state, demand_power_W, total_demand_energy_kWh, number_of_loads, load_priority_factor, daily_energy_Wh, daily_energy_array_Wh = initialize_load_parameters(simulation_time_days, simulation_timestep_hours, load_priority_order, start_day, directory);
    # Recharge data
    daily_real_recharge, daily_real_cost, cost_perkWh, real_recharge_schedule, real_cost_schedule, recharge_amount = initialize_real_recharge_parameters(simulation_time_days, simulation_timestep_hours, total_demand_energy_kWh, recharge_percent, real_recharge_logic);
    ...
    # Time data
    # Horizon
    horizon_time_days = simulation_time_days
    ...
    # Update
    update_time_days = simulation_time_days
    if model_number == 0
        update_time_days = simulation_time_days
    ...
    for t = 1:number_of_timesteps-1
        ...
        if case == 2
            ...
            if t in update_start_timesteps
                ...
                if model_data_number == 1 || model_data_number == 2 || model_data_number == 3 || model_data_number == 3.1
        ...
        elseif case == 1
            if t == 1
                ...
                for k in descending_priority_indices
                    i = i+1
                    virtual_wallet_thresholds[k,t:t+floor(Int,(simulation_time_days*24/simulation_timestep_hours))-1] = 0.05*recharge_amount*(i/number_of_loads)*ones(1,floor(Int,(simulation_time_days*24/simulation_timestep_hours)))
                end
            end
        end
        # Update wallet
    ...
    # Day-wise and load-wise SF, PSF for model_number 1
    #if model_number == 1 
        for d = 1:simulation_time_days
            ...
            for k = 1:number_of_loads
                if daily_load_sf[k,d] == -1
                    daily_psf[d,1] = daily_psf[d,1] + load_priority_factor[k]*100
                else
                    daily_psf[d,1] = daily_psf[d,1] + load_priority_factor[k]*daily_load_sf[k,d]
                end
                if model_daily_load_sf[k,d] == -1
                    model_daily_psf[d,1] = model_daily_psf[d,1] + load_priority_factor[k]*100
                else
                    model_daily_psf[d,1] = model_daily_psf[d,1] + load_priority_factor[k]*model_daily_load_sf[k,d]
                end
            end
        end
        ...
        #CSV.write(directory * "daily_load_sf.csv", daily_load_sf_results_df)
    #end
    ...
    metrics_results[metrics_results_index,4] = priority_sf
    metrics_results[metrics_results_index,5:6] = load_tsf
    metrics_results[metrics_results_index,7] = time_sf
    metrics_results[metrics_results_index,8] = energy_sf
    metrics_results[metrics_results_index,9] = disconnection_counter
    # Add to model metrics matrix
    model_metrics_results_index = model_metrics_results_index + 1
    ...
    model_metrics_results[model_metrics_results_index,5:6] = model_load_tsf
    model_metrics_results[model_metrics_results_index,7] = model_time_sf
    model_metrics_results[model_metrics_results_index,8] = model_energy_sf
    model_metrics_results[model_metrics_results_index,9] = -1
    

    # Generate plots to study model
    if model_data_number == 0
        model_behavior_plots(daily_virtual_recharge, daily_energy_Wh, cost_perkWh, simulation_time_days, horizon_time_days, simulation_results, parameters, directory)
    elseif model_data_number == 1 || model_data_number == 2 || model_data_number == 3 model_data_number == 3.1
        model_behavior_plots(daily_virtual_recharge, daily_energy_Wh_modified, cost_perkWh, simulation_time_days, horizon_time_days, simulation_results, parameters, directory)    
    end
